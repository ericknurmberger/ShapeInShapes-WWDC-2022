...
class PatternScene2: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        createCircle(position: CGPoint(x: (box.frame.midX) + 200, y: box.frame.midY), size: 36.0, color: .black, canMove: true)
        createTriangle(position: CGPoint(x: (box.frame.midX) - 200, y: box.frame.midY), size: 72.0, color: .black, canMove: true)
        createSquare(position: CGPoint(x: 0, y: box.frame.midY), size: CGSize(width: 72.0, height: 72.0), color: .black, canMove: true)
    ...
    /*func createUpsideDownTriangle(position: CGPoint, size: CGFloat, name: String = "triangle", rigthSlotName:String = "slotTriangle", color: SKColor, canMove: Bool = false) {
    ...
    func createDialogue() {
        let texture = SKTexture(imageNamed: "dialogue2pattern\(dialogueIndex)")
    ...
    func createShapesGrid() {
        ...
        for i in 0...5 {
            ...
            for j in 0...3 {
                ...
                default:
                    if objIndex == 0 {
                        createCircle(position: CGPoint(x: posX, y: posY), size: size, rigthSlotName: "cannotMove", color: .black, canMove: false, hasPhysics: false)
                    }
                    else if objIndex == 1 {
                        createSquare(position: CGPoint(x: posX, y: posY), size: CGSize(width: size*2, height: size*2), rigthSlotName: "cannotMove", color: SKColor.black, canMove: false, hasPhysics: false)
                    }
                    else if objIndex == 2 {
                        createTriangle(position: CGPoint(x: posX, y: posY), size: CGFloat(size*2), rigthSlotName: "cannotMove", color: .black, canMove: false, hasPhysics: false)
                    }
                }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            if dialogueIndex > 1 && dialogueIndex < 4 {
                print(dialogueIndex)
                let texture = SKTexture(imageNamed: "dialogue2pattern\(dialogueIndex)")
                ...
                let scene = PatternScene3(size: sceneSize)
